    ...
    def get_game_frame_as_image(frame: int, max_dimension: int = 512) -> Image:
        ...
        with tempfile.TemporaryDirectory(prefix="blmcp-game-frame-") as temporary:
            ...
            code = (
                ...
                "    if _max_dim > 0:\n"
                "        _pct = _rd.resolution_percentage / 100.0\n"
                "        _base_x = max(1, int(round(_rd.resolution_x * _pct)))\n"
                "        _base_y = max(1, int(round(_rd.resolution_y * _pct)))\n"
                "        _largest = max(_base_x, _base_y)\n"
                "        _scale = min(1.0, _max_dim / float(_largest))\n"
                "        _rd.resolution_x = max(1, int(round(_base_x * _scale)))\n"
                "        _rd.resolution_y = max(1, int(round(_base_y * _scale)))\n"
